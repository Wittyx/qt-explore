...
{
    ui->setupUi(this);
    setWindowTitle(QObject::tr("QTreeWidget and QDockWidget Demo"));

    setCentralWidget(ui->scrollArea);                //设置scrollArea为中心控件
    initTree();

    m_labFlie = new QLabel("Current Location:", this);
...
void ExQTreeWidget::initTree()
{
    ...
    root->setText(treeColNum::colItem, QString("Album"));
...
{
    QStringList list = QFileDialog::getOpenFileNames(this, "Select images", "", "Images(*.jpg, *.png, *.*)");         //选择目录
...
{
    m_ratio *= (float)1.2;                                             //在当前比例基础上乘以0.8
